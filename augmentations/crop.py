...
def random_crop(img_tensor, landmarks, crop_range, fill=0., return_crop=False):
    ...
    crop_offsets = torch.hstack(
        (crop_relative[0::2] * img_tensor.size(-1), crop_relative[1::2] * img_tensor.size(-2))).int()

    if return_crop:
        return *crop(img_tensor, landmarks, x1_shift=crop_offsets[0], y1_shift=crop_offsets[2],
                     x2_shift=crop_offsets[1], y2_shift=crop_offsets[3], fill=fill), crop_relative
    return crop(img_tensor, landmarks, x1_shift=crop_offsets[0], y1_shift=crop_offsets[2], x2_shift=crop_offsets[1],
                y2_shift=crop_offsets[3], fill=fill)
...
def crop(img_tensor, landmarks, x1_shift=0, y1_shift=0, x2_shift=0, y2_shift=0, fill=0.):
    ...
    if torch.any(landmarks_p < 0) or torch.any(landmarks_p[0::2] > img_tensor.shape[-1]) or torch.any(
            landmarks_p[1::2] > img_tensor.shape[-2]):
