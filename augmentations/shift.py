...
def random_shift(img_tensor, landmarks, shift_range, fill=0., return_shift=False):
    ...
    shift_offsets = torch.hstack(
        (shift_relative[0] * img_tensor.size(-1), shift_relative[1] * img_tensor.size(-2))).int()

    if return_shift:
        return *shift(img_tensor, landmarks, x_shift=shift_offsets[0], y_shift=shift_offsets[1],
                      fill=fill), shift_offsets
